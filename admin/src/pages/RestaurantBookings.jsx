...
export default function RestaurantBookings() {
    ...
    const fetchBookings = async () => {
        try {
            setLoading(true);
            const response = await hospitalityAPI.getRestaurantBookings();
    ...
    const handleComplete = async (bookingId) => {
        ...
        try {
            ...
            const response = await hospitalityAPI.updateRestaurantBookingStatus(bookingId, 'completed');

            if (response.data && response.data.success) {
                toast.success('Moved to Booking History');
                // Refetch to be sure, but we already updated UI
                fetchBookings();
            } else {
                // Revert on failure
    ...
    const handleDelete = async (bookingId) => {
        ...
        try {
            ...
            const response = await hospitalityAPI.deleteRestaurantBooking(bookingId);
        ...
        <div className="bookings-container">
            <div className="bookings-header">
                <h1 className="bookings-title">
                    {isHistory ? 'Booking History' : 'Active Table Bookings'}
                </h1>
            </div>
